...
%MACRO GRPANALYSIS(TRTGP=);
   ...
   , , , , , ,per GRP SEQ SEQ*GRP SUB(SEQ*GRP) PER(GRP) TRT TRT*GRP,SEQ GRP,SUB(SEQ*GRP))
   RUN;
   %end;
   ...
   %do;
   %PROCGLM(BASE,2,SUB TRT PER SEQ GRP,AUCT,AUCI,CMAX,LAUCT,LAUCI,LCMAX,
   , , , , , ,per GRP SEQ SEQ*GRP SUB(SEQ*GRP) PER(GRP) TRT,SEQ GRP,SUB(SEQ*GRP))
   RUN;
   %end;
...
TITLE5 "&TITLE6";
FOOTNOTE1 "&FOOTNOT1";
SYMBOL1 C=RED I=JOIN V=dot  width=0.5 height=0.5;
SYMBOL2 C=BLUE I=JOIN V=SQUARE width=0.5 height=0.5;

AXIS1 label=(a=90 "&label1");

PROC GPLOT DATA=CMEAN UNIFORM;
