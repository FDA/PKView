define('tools/pkview/components/home/newProject/newProject', [
    'knockout',
    'koSelectize',
    'durandal/app',
    'plugins/dialog',
...
function (ko, koSelectize, app, dialog, net, txt, files, modal, analysis) {
    ...
    var ctor = function () {
        ...
        self.projectName = ko.observable("New pk analysis"); // New project name
        self.AnalysisType = ko.observable("SafetyAnalysis");
        ...
        self.refreshSubmissions = function () {
            ...
            $.get('/api/submissions').then(function (submissions) {
        ...
        self.createProject = function () {
            ...
            if ($.trim(self.selectedSubmission()).length == 0) {
                app.showMessage('Please select a submission from the list', 'PkView');
                return;
            }

            if ($.trim(self.projectName()).length == 0) {
                app.showMessage('Project name cannot be empty', 'PkView');
                return;
            }
            ...
            if (self.AnalysisType() == "SafetyAnalysis") {
                self.done({
                    name: self.projectName().trim(),
                    submissionType: submissionType,
                    submissionNumber: submissionNumber,
                    AnalysisType: "SafetyAnalysis"
                });
            }
            else {
