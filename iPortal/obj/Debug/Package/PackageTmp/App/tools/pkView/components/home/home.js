...
function (ko, koSelectize, app, dialog, net, modal, filter) {
    ...
    var ctor = function () {
        ...
        self.newProject = function () {
            var newProjectDialog = new modal({
                ...
                activationData: {
                        ...
                        if (projectSettings.AnalysisType == "SafetyAnalysis") {
                            self.saveSafetyProject();
                            self.done(5);
                        }
                        else {
                            self.done(1);
        ...
        // Save safety analysis project
        self.saveSafetyProject = function () {
            net.ajax({
                url: "/api/pkview/saveSafetyProject",
                data: { projectName: self.projectName(), submissionId: self.submission() },
                type: "GET",
        ...
        // Make a Safety analysis
        self.SafetyAnalysis = function (submission, project) {
            var profileName = project;
            self.projectName(profileName);
            self.submission(submission);
            self.done(5);
