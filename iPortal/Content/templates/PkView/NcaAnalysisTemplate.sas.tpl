...
DATA QCPK;
	retain sub seq per trt QCAUCI QCAREA QCCMAX;
	SET PK1;
	QCAREA=AUCT;
	QCAUCI=AUCI;
	QCCMAX=CMAX;
...
run;
PROC PRINT DATA=QCPK; RUN;

PROC SORT DATA=QCPK; BY SUB PER TRT; RUN;
...
RAUCT=FIRMAREA/QCarea;
RAUCI=FIRMAUCI/QCAUCI;
RCMAX=FIRMCMAX/QCCMAX;
run;
proc print;RUN;
...
proc transpose data=boxplot out=longboxplot(rename=(_NAME_=RParameter  COL1=QC_vs_FIRM))  ;
...
label trt="Treatment Group" QC_vs_FIRM="FIRM vs QC Ratio" rparameter="Parameter";
vbox QC_vs_FIRM/category=trt group=Rparameter;
run;
