define('tools/pkView/components/review/VariabilityMetaAnalysis', [
        'knockout',
        'koSelectize',
    ...
    var ctor = function () {
        ...
        self.createForestPlotReport = function () {
            var newReportDialog = new modal({
                title: "New Variability Report",
                model: "tools/pkView/components/report/newReport/newvariabilityReport",
                activationData: {
        ...
        self.setMetaFormat = function (study, data) {
            ...
            var setMetaFormatDialog = new modal({
                ...
                model: "tools/pkview/components/review/sections/metaVariabilityFormat",
                activationData: {
                    study: study,
                    data: data                    
                },
            ...
            dialog.show(setMetaFormatDialog)
                .then(function (result) {
                    if (result)
                    {
                        if (result.showdistributionornot == "saveandgenerate")
                        {
                            self.generateMetaAnalysis();
                        }
                        else if (result.showdistributionornot == "showdistribution")
                        {
                            self.generateMetaAnalysis();
                        }
                        else return;
        ...
        self.downloadVariabilityMetaPackage = function () {

            // Do nothing if download function is already in progress
            if (self.metacomputingReport()) return false;
            self.metacomputingReport(true);
            // Create and download results package
            analysis.createVariabilityMetaPackage({
                NDAName: self.NDAName,
                activeSupplement: self.activeSupplement,
                ProfileName: self.ProfileName,
